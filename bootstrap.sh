...
docker run --rm -ti postgres:10 --network host -e PGPASSWORD=ckan pg_restore -d postgres -v -h lh -p 5432 -U ckan  --if-exists  --clean postgres-prod-dump.custom --create --no-owner --exit-on-error
