...
function CategoryIcon({url, active}){
    return(
        <Image width="50px" src={url} />
    )
}

export default function DatabaseCard({name, categories=[], organization, tags, size, tableNum, externalLinkNum, updatedSince, updatedAuthor}){
    const databaseInfo = [];
    ...
    return(
        <Card icons={categories.map(c => <CategoryIcon url={`/next-img/categories/${c}.png`}/>)} spacing={2}>
            <Title>{name}</Title>
            ...
            <Subtitle fontSize="12px" fontStyle="italic">Atualizado há {updatedSince} por {updatedAuthor}</Subtitle>
        </Card>
    )
}