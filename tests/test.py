...
CKAN_API_KEY = (
    ...
    or open("configs/ckan-dev-api-token.sh").read().split("=")[-1].strip()
)
...
def test():
    ret = requests.get(
        CKAN_URL + "/api/3/action/package_show?name_or_id=br-geobr-mapas"
    )
    assert ret.status_code == 200, ret.json()
    package = ret.json()["result"]

    ret = requests.post(CKAN_URL + "/api/3/action/bd_dataset_validate", json=package)
    assert ret.status_code == 200, ret.json()
    assert ckan.action.bd_dataset_validate(**package)
