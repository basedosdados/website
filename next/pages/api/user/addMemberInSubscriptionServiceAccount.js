  ...
  try {
    const res = await axios({
      ...
      data: {
        query: `
          mutation {
            addStripeSubscriptionServiceAccount(email: "${email}", subscriptionId: "${subscriptionId}") {
              ok
              errors
            }
          }
    ...
    return res.data?.data?.addStripeSubscriptionServiceAccount
  } catch(err) {
    return "err"
  }
}
...
export default async function handler(req, res) {
  ...
  const result = await addMemberInSubscriptionServiceAccount(atob(req.query.r), atob(req.query.p), token())
