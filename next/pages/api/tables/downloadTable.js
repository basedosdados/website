...
async function downloadTable(url, datasetID, tableId, token, res) {
  ...
  const urlDownloadClosed = process.env.URL_DOWNLOAD_CLOSED.replace(
    "gs://",
    "",
  );
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=${datasetID}_${tableId}.csv.gz`,
    );
...
export default async function handler(req, res) {
  ...
  if (atob(req.query.d) === "false")
    return res
      .status(500)
      .json({ error: "Você não tem permissão para fazer esse download" });
