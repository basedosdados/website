...
export default function SearchPage({
    ...
    const organizations = data ? .organizations ?
        ...
        .map((t) => ({
            name: t,
            displayName: data.organizations_display_names[t] + ` (${data.organizations[t]})`,
    ...
    const entities = data ? .entities ?
        ...
        .map((t) => ({
            name: t,
            displayName: availableOptionsTranslations[t] + ` (${data.entities[t]})`,
    ...
    const updateFrequencies = data ? .update_frequencies ?
        ...
        .map((t) => ({
            name: t,
            displayName: availableOptionsTranslations[t] + ` (${data.update_frequencies[t]})`,
    ...
    const spatialCoverages = {
        Continente: data ? .spatial_coverage_continent ?
            ...
            .map((t) => ({
                name: t,
                displayName: availableOptionsTranslations[t] +
        ...
        País: data ? .spatial_coverage_country ?
            ...
            .map((t) => ({
                name: t,
                displayName: availableOptionsTranslations[t] + ` (${data.spatial_coverage_country[t]})`,
    ...
    return ( <
            ...
            /SectionTitle> <
            CheckboxFilterAccordion isActive = {
                (paramFilters.resource_type || []).length > 0 }
            alwaysOpen choices = {
                [{
                        key: "bdm_table",
                        name: ( <
            ...
            /> <
            CheckboxFilterAccordion canSearch = { true }
            isActive = {
                (paramFilters.organization || []).length > 0 }
            ...
            CheckboxFilterAccordion canSearch = { true }
            isActive = {
                (paramFilters.spatial_coverage || []).length > 0 }
            choices = {
                [...spatialCoverages.Continente] }
            ...
            CheckboxFilterAccordion canSearch = { true }
            isActive = {
                (paramFilters.obs_level_entity || []).length > 0 }
            choices = { entities }
            ...
            CheckboxFilterAccordion canSearch = { true }
            isActive = {
                (paramFilters.update_frequency || []).length > 0 }
            ...
            width = "100%" >
            <
            DebouncedControlledInput value = { search }
            onChange = {
                (val) => {
                    setSearch(val);
                }
            ...
            letterSpacing = "0.1em" >
            { data ? .count || "..." }
            conjunto(s) encontrado(s) { " " } { search ? " para " + search : "" } <
            /Heading> {
                userData ? .is_admin ? ( <
                    Button w = "170px"
                ...
                { base: "column", lg: "row" } } >
            {
                ...
                .map(([k, values]) => ( <
                    FilterTags label = { fieldTranslations[k] }
                    fieldName = { k }
                    values = {
                        k === "temporal_coverage" ?
