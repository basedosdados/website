import { Stack, VStack, HStack, Box, Skeleton } from "@chakra-ui/react";
...
import { mdxComponents } from "../../components/organisms/Blog/Slug";
...
export default function CaseStudies({ serialize }) {
  ...
  return (
    <MainPageTemplate paddingX="24px">
      <Head>
        <title>
          {t("pageTitle", { title: frontmatter.displayTitle || "" })}
        </title>
        <link rel="image_src" href={frontmatter.thumbnail || ""} />
        <meta
          property="og:image"
      ...
      <Stack spacing={0} maxWidth="1440px" margin="50px auto auto">
        <Link
          ...
          href={"/services"}
        >
          <ChevronIcon alt="back" width="16px" transform={"rotate(180deg)"} />
          {t("backLink")}
        </Link>
        ...
        >
          ...
          >
            ...
            <LabelText>{t("about")}</LabelText>

            <BodyText as="h2" paddingBottom="32px" color="#71757A">
              {frontmatter?.about || ""}
            </BodyText>

            <LabelText>{t("sector")}</LabelText>

            <BodyText paddingBottom="32px" color="#71757A">
              {frontmatter?.sector || ""}
            </BodyText>

            <BodyText paddingBottom="8px">{t("contactText")}</BodyText>
