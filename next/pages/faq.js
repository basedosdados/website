import { Box, VStack, Stack, Divider, Collapse } from "@chakra-ui/react";
...
const QuestionsBox = ({ question, answer, id, active }) => {
  ...
  return (
    <Stack spacing={0} className={styles.questionContainer}>
      <Box
        display="flex"
        cursor="pointer"
        marginBottom="24px"
      ...
      >
        <LabelText typography="x-large">{question}</LabelText>
        <CrossIcon
...
export default function FAQ({ faqs }) {
  ...
  const filterByCategory = (category) => {
    return allQuestions.filter((elm) => elm.categories.includes(category));
  };

  useEffect(() => {
    if (categorySelected)
      return setQuestions(filterByCategory(categorySelected));
  }, [categorySelected]);
  ...
  useEffect(() => {
    if (searchFilter.trim() === "") {
      setQuestions(
        categorySelected ? filterByCategory(categorySelected) : allQuestions,
      );
    } else {
