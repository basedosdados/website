...
export default function DataAPIDocs() {
    ...
    async function fetchData() {
      try {
        ...
        const [categoriesRes, endpointsRes] = await Promise.all([
          fetch(`/api/dataAPI/getDataAPIEndpointCategories?locale=${locale}`, {
            method: "GET",
          }),
          fetch(`/api/dataAPI/getDataAPIEndpoints?locale=${locale}`, {
            method: "GET",
          }),
        ]);
        ...
        const processedCategories = Object.values(
          categoriesData?.resource || {},
        );
  ...
  useEffect(() => {
    if (selectedEndpoint && endpoints.length > 0) {
      const selected = endpoints.find(
        (endpoint) => endpoint._id === selectedEndpoint,
      );
  ...
  return (
    <MainPageTemplate>
      <Text fontSize="4xl" fontWeight="bold" textAlign="center" mb={8} mt={4}>
        {t("title")}
      </Text>
      <Grid
            ...
            <Box>
              ...
              <VStack align="stretch" pl={4} mb={6}>
                <Text
                  ...
                  color={
                    !selectedEndpoint && selectedSection === "overview"
                      ? "blue.500"
                      : "inherit"
                  }
                  onClick={() => {
                ...
                <Text
                  ...
                  _hover={{ color: "blue.500" }}
                  color={
                    !selectedEndpoint && selectedSection === "authentication"
                      ? "blue.500"
                      : "inherit"
                  }
                  onClick={() => {
                ...
                <Text
                  ...
                  _hover={{ color: "blue.500" }}
                  color={
                    !selectedEndpoint && selectedSection === "pricing"
                      ? "blue.500"
                      : "inherit"
                  }
                  onClick={() => {
                  ...
                  <Box key={category._id}>
                    ...
                    <VStack align="stretch" pl={4} mb={4}>
                      {endpoints
                        .filter(
                          (endpoint) =>
                            endpoint.category?._id === category._id &&
                            endpoint.isActive,
                        )
                        .map((endpoint) => (
                          <Text
                          ...
                          >
                            {endpoint[`name${capitalize(locale)}`] ||
                              endpoint.name}
                          </Text>
                        ))}
                    </VStack>
            ...
            endpointDetails && (
              <VStack align="stretch" spacing={6}>
                <Text fontSize="2xl" fontWeight="bold">
                  {endpointDetails[`name${capitalize(locale)}`] ||
                ...
                <Box>
                  ...
                  <Text>
                    {endpointDetails[`description${capitalize(locale)}`] ||
                ...
                <Box>
                  <Text fontWeight="bold" mb={2}>
                    {t("endpoints.endpointURL")}
                  </Text>
                  <Code p={4} borderRadius="md" display="block">
                    https://api.basedosdados.org/data?category=
                    {endpointDetails.category.slug}&endpoint=
                    {endpointDetails.slug}&parameters=
                    {"<PARAMETER1:VALUE1,PARAMETER2:VALUE2>"}&key={"<API_KEY>"}
                  </Code>
                </Box>

                {/* Parameters */}
                {endpointDetails.parameters &&
                  Object.keys(endpointDetails.parameters).length > 0 && (
                    <Box>
                      <Text fontWeight="bold" fontSize="lg" mb={4}>
                        {t("endpoints.parameters")}
                      </Text>
                      <Grid
                        templateColumns="repeat(auto-fit, minmax(300px, 1fr))"
                        gap={4}
                      >
                        {Object.values(endpointDetails.parameters).map(
                          (param) => (
                            <Box
                              key={param._id}
                              p={4}
                            ...
                            >
                              ...
                              <Text>
                                {param[`description${capitalize(locale)}`] ||
                                  param.description}
                              </Text>
                            </Box>
                          ),
                ...
                {/* Pricing Tiers */}
                {endpointDetails.pricingTiers &&
                  Object.keys(endpointDetails.pricingTiers).length > 0 && (
                    <Box>
                      <Text fontWeight="bold" fontSize="lg" mb={4}>
                        {t("endpoints.pricingTiers")}
                      </Text>
                      <Grid
                        templateColumns="repeat(auto-fit, minmax(250px, 1fr))"
                        gap={4}
                      >
                        {Object.values(endpointDetails.pricingTiers).map(
                          (tier) => (
                            <Box
                              key={tier._id}
                              p={4}
                            ...
                            >
                              <Text fontWeight="semibold" mb={2}>
                                {tier.minRequests} - {tier.maxRequests || "∞"}{" "}
                                {t("endpoints.requests")}
                              </Text>
                              <Text>
                                {tier.pricePerRequest}{" "}
                                {tier.currency?.[`name${capitalize(locale)}`] ||
                                  tier.currency?.name}{" "}
                                / {t("endpoints.request")}
                              </Text>
                            </Box>
                ...
                <Text fontSize="sm" color="gray.600" mt={4}>
                  {t("endpoints.lastUpdated")}:{" "}
                  {new Date(endpointDetails.updatedAt).toLocaleDateString()}
                </Text>
              </VStack>
            )
          ) : (
              ...
              {selectedSection === "overview" ? (
                <>
                  ...
                  >
                    ...
                    <Text>{t("overview.earlyReleaseDescription")}</Text>
                  </Box>
                  <Text fontSize="2xl" fontWeight="bold">
                    {t("overview.title")}
                  </Text>
                  <Text>{t("overview.welcome")}</Text>
                  <Text>{t("overview.description")}</Text>
                  <Text>{t("overview.descriptionPricing")}</Text>
                  <Box>
                    <Text fontWeight="bold" mb={2}>
                      {t("overview.baseURL")}
                    </Text>
                ...
                <>
                  ...
                  <Text>{t("authentication.description")}</Text>
                  <Box>
                    <Text fontWeight="bold" mb={2}>
                      {t("authentication.apiKey")}
                    </Text>
                    <Text mb={4}>{t("authentication.getApiKeyBy")}</Text>
                    <VStack align="stretch" spacing={2} pl={4}>
                      <Text>1. {t("authentication.getApiKeyBy1")}</Text>
                      <Text>
                        2. {t("authentication.getApiKeyBy2Contactus")}{" "}
                        <Link href="/contact">
                    ...
                    <br></br>
                    <Text>{t("authentication.weWillAnswerYou")}</Text>
                  </Box>
                </>
              ) : (
                <>
                  ...
                  <Text>{t("pricing.description")}</Text>
                  <Box>
                    <Text mb={4}>{t("pricing.addCredits")}</Text>
                    <VStack align="stretch" spacing={2} pl={4}>
