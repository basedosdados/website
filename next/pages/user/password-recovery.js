import { Stack, FormControl, List } from "@chakra-ui/react";
...
export default function PasswordRecovery({ confirmed, uid, confirmToken }) {
  ...
  async function handleEmailPasswordRecovery(value) {
    ...
    if (!regexEmail.test(value))
      return setError(t("passwordRecovery.invalidEmail"));

    const API_URL = `${process.env.NEXT_PUBLIC_API_URL}`;

    const getIdUser = await fetch(`/api/user/getIdUser?p=${btoa(value)}`, {
      method: "GET",
    }).then((res) => res.json());
  ...
  async function handleSubmit(event) {
    ...
    if (!formData.confirmPassword) {
      validationErrors.confirmPassword = t(
        "passwordRecovery.confirmPasswordRequired",
      );
    }
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <FormControl
              isInvalid={!!errors.password || !!errors.regexPassword}
            >
            ...
            <FormControl marginTop="40px" isInvalid={!!errors.confirmPassword}>
              <LabelTextForm
                text={t("passwordRecovery.confirmPasswordLabel")}
              />
              <InputForm
  ...
  return (
    ...
    >
      ...
      >
        ...
        <BodyText typography="large" textAlign="center" color="#464A51">
        ...
        <Button
          ...
          _hover={{
            backgroundColor: forwardingDisabled ? "#2B8C4D" : "#22703E",
          }}
        ...
        >
          {forwardingDisabled
            ? t("passwordRecovery.waitSeconds", { count })
            : t("passwordRecovery.sendResetEmail")}
        </Button>

        <BodyText typography="small" textAlign="center">
