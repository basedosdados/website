...
export async function getServerSideProps(context) {
  ...
  const result = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL_FRONTEND}/api/user/activeAccount?q=${query.q}&p=${query.p}`,
    { method: "GET" },
  ).then((res) => res.json());
  const data = result;
  ...
  return (
    <MainPageTemplate display="flex" justifyContent="center" cleanTemplate>
      ...
      >
        ...
        {data?.success === true ? (
          <VStack spacing={4}>
            <Display typography="small" textAlign="center">
              {t("activate.accountActive")}
            </Display>
            ...
            <Link href="/user/login">
              <Button onClick={() => {}}>{t("activate.loginToAccount")}</Button>
            </Link>
          </VStack>
        ) : (
          <VStack spacing={4}>
            <Display typography="small" textAlign="center">
              {t("activate.somethingWentWrong")}
            </Display>

            <SectionText textAlign="center">
              {t("activate.activationProblem")}
            </SectionText>
            <Link href="/contact">
              <Button onClick={() => {}}>{t("activate.contactUs")}</Button>
            </Link>
          </VStack>
        )}
      </Stack>
