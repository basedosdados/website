...
function LatestBlogCard({ slug, frontmatter }) {
  const { title, description, date, authors, categories, thumbnail } = frontmatter;
  ...
  const thumbnailSrc = (typeof thumbnail === "string" && thumbnail.trim().length > 0)
    ? thumbnail
    : (categoryThumbnails[categories?.[0]] || "https://storage.googleapis.com/basedosdados-website/blog/um-site-feito-a-varias-maos/image_9.png");
