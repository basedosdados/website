  ...
  async function handleSubmit(event) {
    ...
    if (formData.password !== "") {
      const result = await fetch(
        `/api/user/getToken?a=${btoa(userInfo.email)}&q=${btoa(formData.password)}&s=${"true"}`,
        { method: "GET" },
      ).then((res) => res.json());
      if (result.error) {
    ...
    if (Object.keys(validationErrors).length > 0) {
      ...
      const result = await fetch(
        `/api/user/updatePassword?b=${btoa(id)}&p=${btoa(formData.newPassword)}`,
        { method: "GET" },
      ).then((res) => res.json());
    ...
    <Stack spacing="0" maxWidth="480px">
      ...
      >
        ...
        <Stack
          ...
          margin="24px 0"
        >
          <SuccessIcon width="90px" height="90px" fill="#34A15A" />
          <TitleText>{t("username.passwordChangedSuccessfully")}</TitleText>
        </Stack>

        <Button width="100%" onClick={() => window.open("/", "_self")}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <FormControl
          marginTop="24px"
          isInvalid={!!errors.newPassword || !!errors.regexPassword}
        >
