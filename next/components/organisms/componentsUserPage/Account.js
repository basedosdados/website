...
export default function Account({ userInfo }) {
  ...
  async function submitUpdate() {
    ...
    if (formData.username === "")
      return setErrors({ username: t("username.invalidUsername") });
    if (formData.username.includes(" "))
      return setErrors({ username: t("username.noSpacesInUsername") });
    setIsLoading(true);
    ...
    const result = await fetch(
      `/api/user/updateUser?p=${btoa(id)}&q=${btoa(form.username)}`,
      { method: "GET" },
    ).then((res) => res.json());
  ...
  async function validateAccountDeletion(id) {
    ...
    const hasActiveSubscription =
      userInfo?.internalSubscription?.edges?.[0]?.node?.canceledAt === null;
    ...
    if (userInfo?.proSubscription === "bd_pro_empresas") {
      const res = await fetch(`/api/user/getMembers?p=${btoa(id)}`, {
        method: "GET",
      });
      const membersData = await res.json();

      if (
        membersData?.internalSubscription?.edges?.[0]?.node?.subscribers?.edges
          ?.length > 0
      ) {
        setHasMembers(true);
  ...
  async function deleteUserAccount(id) {
    const result = await fetch(`/api/user/deleteAccount?p=${btoa(id)}`, {
      method: "GET",
    }).then((res) => res.json());
  ...
  return (
    <Stack spacing="24px">
      ...
      >
        <Stack spacing={0} marginBottom="16px">
          <TitleText marginRight="20px">
            {t("username.confirmAccountDeletion")}
          </TitleText>
          <ModalCloseButton
            fontSize="14px"
            top="34px"
      ...
      >
        ...
        <Stack width={{ base: "100%", lg: "fit-content" }}>
          <Button
            width="100%"
            backgroundColor="#BF3434"
            _hover={{
      ...
      >
        ...
        <Stack spacing="4px" marginBottom="24px" flexDirection="column">
          {hasCancelSubscription && (
            <ExtraInfoTextForm>
              {t("username.errorEraseAccountText")}
            </ExtraInfoTextForm>
          )}
          {hasMembers && (
            <ExtraInfoTextForm>
        ...
        <Stack spacing={0} width={{ base: "100%", lg: "fit-content" }}>
          <Button
            whiteSpace="nowrap"
            width="100%"
            backgroundColor="#BF3434"
      ...
      <Box>
        <TitleTextForm color="#BF3434">
          {t("username.deleteAccount")}
        </TitleTextForm>
        <ExtraInfoTextForm color="#71757A">
          {t("username.accountAccessWarning")}
        </ExtraInfoTextForm>
        <Button
          backgroundColor="#BF3434"
          _hover={{
