...
export default function PlansAndPayment({ userData }) {
  ...
  const [emailGCP, setEmailGCP] = useState(
    userData?.gcpEmail || userData?.email,
  );
  ...
  const subscriptionInfo = () => {
    if (userData?.internalSubscription?.edges?.[0]?.node)
      return userData?.internalSubscription?.edges?.[0]?.node;
    if (userData?.subscriptionSet?.edges?.[0]?.node)
      return userData?.subscriptionSet?.edges?.[0]?.node;
  };
  ...
  const resources = {
    "BD Gratis": {
      ...
      resources: [
        ...
        {
          name: t("username.integratedData"),
          tooltip: t("username.integratedDataTooltip"),
        },
        ...
        planActive
          ? ""
          : {
              name: t("username.downloadLimit100MB"),
              tooltip: t("username.downloadLimit100MBTooltip"),
            },
      ],
    },
    bd_pro: {
      ...
      resources: [
        ...
        {
          name: t("username.downloadLimit1GB"),
          tooltip: t("username.downloadLimit1GBTooltip"),
        },
      ],
    },
    bd_pro_empresas: {
  ...
  async function cancelSubscripetion() {
    ...
    const subscriptionActive = await fetch(
      `/api/stripe/getSubscriptionActive?p=${btoa(id)}`,
      { method: "GET" },
    ).then((res) => res.json());

    const result = await fetch(
      `/api/stripe/removeSubscription?p=${btoa(subscriptionActive)}`,
      { method: "GET" },
    ).then((res) => res.json());
    ...
    const user = await fetch(`/api/user/getUser?p=${btoa(id)}`, {
      method: "GET",
    }).then((res) => res.json());
  ...
  async function closeModalSucess() {
    ...
    while (
      !user?.internalSubscription?.edges?.[0]?.node &&
      attempts < maxAttempts
    ) {
      user = await fetch(`/api/user/getUser?p=${btoa(id)}`, {
        method: "GET",
      }).then((res) => res.json());
  ...
  function changeIntervalPlanCheckout() {
    ...
    const value = Object.values(plans).find(
      (elm) =>
        elm.interval === togglerValue &&
        elm.productSlug === checkoutInfos?.productSlug,
    );
    setCheckoutInfos(value);
    setCoupon("");
    setValueCoupon("");
  ...
  async function validateStripeCoupon() {
    ...
    const result = await fetch(
      `/api/stripe/validateStripeCoupon?p=${btoa(plan)}&c=${btoa(valueCoupon)}`,
      { method: "GET" },
    ).then((res) => res.json());
  ...
  const CouponDisplay = () => {
    ...
    if (couponInfos?.duration === "once")
      limitText = toggleAnual
        ? t("username.validFor1Year")
        : t("username.validFor1Month");
    if (couponInfos?.duration === "repeating")
      limitText = `${t("username.validFor")} ${couponInfos?.durationInMonths} ${couponInfos?.durationInMonths.length === 1 ? t("username.month") : t("username.months")})`;

    return (
      <>
        <GridItem>
          <Text>
            {t("username.coupon")} {coupon.toUpperCase()} {limitText}
          </Text>
        </GridItem>
        <GridItem textAlign="end">
          <Text>
            -{" "}
            {couponInfos?.discountAmount?.toLocaleString("pt-BR", {
              style: "currency",
              currency: "BRL",
  ...
  const TotalToPayDisplay = () => {
    ...
    if (couponInfos?.discountAmount) {
      value = (
        checkoutInfos?.amount - couponInfos?.discountAmount
      ...
      value = checkoutInfos?.amount?.toLocaleString("pt-BR", {
        style: "currency",
        currency: "BRL",
        minimumFractionDigits: 2,
      });
    ...
    return (
      <>
        <GridItem>
          <Text color="#252A32" fontWeight="500">
            {t("username.totalToPay")}
          </Text>
        </GridItem>
        <GridItem textAlign="end">
          <Text color="#252A32" fontWeight="500">
            {value}/{formattedPlanInterval(checkoutInfos?.interval, true)}
          </Text>
        </GridItem>
      </>
    );
  ...
  async function handlerEmailGcp() {
    ...
    const response = await fetch(
      `/api/user/changeUserGcpEmail?p=${btoa(emailGCP)}`,
    ).then((res) => res.json());
    ...
    <Stack spacing={0}>
          ...
          <Stack flex={1} spacing="32px">
            <Stack flexDirection="column" spacing={0} gap="16px">
              <Box display="flex" flexDirection="row" gap="8px" width="100%">
                <LabelText>{checkoutInfos?.productName}</LabelText>
                <BodyText
                  cursor="pointer"
                  color="#0068C5"
                  _hover={{ color: "#0057A4" }}
      ...
      >
        ...
        >
          ...
          <TitleText>{t("username.congratulations")}</TitleText>
          <BodyText color="#464A51">
              ...
              button={{
                ...
                onClick:
                  userData?.proSubscription === "bd_pro"
                    ...
                    : () => {
                        setPlan(
                          plans?.[`bd_pro_${toggleAnual ? "year" : "month"}`]
                            ._id,
                        );
                        PlansModal.onClose();
                        EmailModal.onOpen();
                ...
                isCurrentPlan:
                  userData?.proSubscription === "bd_pro" ? true : false,
              }}
            />
                ...
                onClick:
                  userData?.proSubscription === "bd_pro_empresas"
                    ...
                    : () => {
                        setPlan(
                          plans?.[
                            `bd_empresas_${toggleAnual ? "year" : "month"}`
                          ]._id,
                        );
                        PlansModal.onClose();
                        EmailModal.onOpen();
              ...
              <BodyText typography="small" color="#71757A">
                ...
                <Text as="span" fontWeight="500" color="#464A51">
                  {formatTimeStamp(
                    subscriptionInfo()?.canceledAt
                      ? subscriptionInfo()?.canceledAt
                      : subscriptionInfo()?.nextBillingCycle,
                  )}
                </Text>
              </BodyText>
            </Box>
