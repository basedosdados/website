...
function SearchColumn({ isLoaded, resource, columns }) {
  ...
  return (
    <ControlledInputSimple
      ...
      maxWidth="100%"
      width="100%"
      icon={<SearchIcon alt="pesquisar" width="16.8px" height="16.8px" />}
    />
  );
}
...
export default function TableColumns({
  ...
  const isUserPro = () => {
    ...
    if (user?.internalSubscription?.edges?.[0]?.node?.isActive === true)
      return true;
    return false;
  };
  ...
  const handleCheckboxChange = (columnSlug) => {
    if (isChecked(columnSlug)) {
      onChangeCheckedColumns(
        checkedColumns.filter((slug) => slug !== columnSlug),
      );
    } else {
  ...
  const headers = [
    ...
    },
    {
      pt:
        template === "download"
          ? t("column.translationTable")
          : t("column.needsTranslation"),
      tooltip:
        template === "download"
          ? t("column.translationTableTooltip")
          : t("column.needsTranslationTooltip"),
    },
    {
  ...
  function DictionaryDownload() {
    async function downloadTable() {
      const result = await fetch(
        `/api/tables/getDictionaryTable?p=${btoa(query.dataset)}`,
        { method: "GET" },
      ).then((res) => res.json());
      ...
      window.open(
        `/api/tables/downloadTable?p=${btoa(cloudTables?.gcpDatasetId)}&q=${btoa(cloudTables?.gcpTableId)}&d=${btoa(downloadInfo.downloadPermitted)}&s=${btoa(downloadInfo.downloadWarning)}`,
      );
    }
  ...
  function TranslationTable({ value, dictionary }) {
    const downloadInfo = HasDownloadPermitted(
      value?.table?.uncompressedFileSize,
    );
    ...
    if (gcpDatasetID === "br_bd_diretorios_brasil") {
      if (gcpTableId === "empresa" || gcpTableId === "cep")
        return t("column.noTranslationNeeded");
    }
    ...
    return (
      <Box>
        ...
        {dictionary === true && <DictionaryDownload />}
      </Box>
    );
  }
  ...
  const measurementUnit = (value) => {
    ...
    return <Latex>{`$${translated(splitValue).join("")}$`}</Latex>;
  };
  ...
  function TranslationColumnException({ value }) {
    const cloudValues =
      value?.node?.directoryPrimaryKey?.table?.cloudTables?.edges?.[0]?.node;
    const gcpDatasetID = cloudValues?.gcpDatasetId || "";
    const gcpTableId = cloudValues?.gcpTableId || "";
            ...
            <Thead role="rowgroup" position="relative">
              <Tr>
                  ...
                  >
                    <Box padding="14px 22px 14px 30px">
                      <Checkbox
                        ...
                        isIndeterminate={
                          checkedColumns.length !== resource.length &&
                ...
                <TableHeader
                  ...
                  left={{
                    base: "none",
                    lg: template === "download" ? "0" : "72px",
                  }}
                />

                {headers.map(
                  (elm, i) => i != 0 && <TableHeader key={i} header={elm} />,
                )}
              </Tr>
            </Thead>
              ...
              {columns.length > 0 &&
                columns.map((elm, i) => (
                  <Tr key={i} role="row" borderBottom="1px solid #DEDFE0">
                    {template === "checks" && (
                      ...
                      >
                        ...
                        >
                          <Checkbox
                            isChecked={isChecked(
                              elm.node[`name${capitalize(locale)}`] ||
                                elm.node.name,
                            )}
                            onChange={() =>
                    ...
                    <TableValue>
                      {elm?.node?.[`description${capitalize(locale)}`] ||
                        elm?.node?.description ||
                        t("column.notProvided")}
                    </TableValue>

                    <TableValue>
                      {elm?.node?.bigqueryType?.name
                        ? elm.node.bigqueryType.name
                        : t("column.notProvided")}
                    </TableValue>

                    <TableValue>
                      {elm?.node?.temporalCoverage?.start &&
                      elm?.node?.temporalCoverage?.end
                        ? elm.node.temporalCoverage.start +
                          " - " +
                          elm.node.temporalCoverage.end
                        : t("column.notProvided")}
                    ...
                    <TableValue>
                      {elm?.node?.containsSensitiveData === true
                        ? t("column.yes")
                        : elm?.node?.containsSensitiveData === false
                          ? t("column.no")
                          : t("column.notProvided")}
                    </TableValue>

                    <TableValue>
                      {elm?.node?.[`observations${capitalize(locale)}`] ||
                        elm?.node?.observations ||
                        t("column.notProvided")}
                    </TableValue>
                  </Tr>
                ))}
