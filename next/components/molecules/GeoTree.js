...
export function GeoTree(props, schema) {
    ...
    const update_chosen_levels = (level) => (e) => {
        ...
        for (let i = 0; i < level; i++)
            new_chosen_levels[i] = chosen_levels[i];
    ...
    for (let i = 0; i < n_levels; i++) {
        ...
        selects.push(
            <select className="form-control" onChange={update_chosen_levels(i)} value={chosen_levels[i]}>
                {options}
            </select>
        );
    }
    ...
    return (
        <div>
            <label> </label>
            {selects}
            <input readOnly className={props.classNames}
                name={formid}
                label="spatial_coverage_hidden_field"
                type="text" hidden={false} value={props.formData}>
            </input>
        </div>
    );
}
...
function _id_to_levels(id, n_levels) {
    if (id === undefined || id === null)
        return Array(n_levels).fill('');
    let out = [];
    for (let i = 0; i < id.length; i++)
        if (id[i] === '.')
            out.push(id.slice(0, i));
    out.push(id);
    while (out.length < n_levels)
        out.push('');
    return out;
}
