...
class Migrate:
    ...
    def ckan_api(self):
        ...
        user_agent = None
        CKAN_API_KEY = CKAN_API_KEY='eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI1RUxycHVUV0ZRMXoxbXAwYWE5RjNNalpsZnlBbUpGZ3phUEZIYWxTTXg0eUxReV9UdnJjOHRudFhNeVNneHlLTWJSSmVNZGt4U0NwenU4QSIsImlhdCI6MTYyMTEyMzM5NH0.ePC4i_UpPK-98MCLV-pVjRD2cw1dykPFXeJpxNVUPwY'
    ...
    def _update_spatial_coverage(self, resource_dict):
        ...
        if isinstance(spatial_coverage, dict):
            for key in spatial_coverage.keys():
                spatial_coverage[key] = spatial_coverage_mapping[key][spatial_coverage[key]]
    ...
    def migrate_resource_external_link(self, resource_dict, package_dict):
        ...
        if temporal_coverage:
            resource_dict['temporal_coverage'] = [v for v in temporal_coverage if isinstance(v, int)] 
    ...
    def migrate_package_extras(self, package_dict):
        ...
        if extras:
            ...
            to_resources_args = [
                ...
                {
                    'current_key': ['estado', 'pais', 'regiao'], # keep alphabetical order
                    'new_key': 'spatial_coverage',
                    'resource_types': ['external_link', 'bdm_table'],
                    'transform_func': lambda x, y, z: {'admin1': x, 'country': y, 'continent': z}
                },
                # 'gratis' --> is_free (external_link)
                {
                    'current_key': 'gratis',
    ...
    def to_resource(self, package_dict, extras_dict, current_key, new_key, resource_types, transform_func):
        
        obj = [extras_dict.pop(key) for key in sorted(extras_dict.keys()) if key in current_key]
        if len(obj) == 1:
    ...
    def migrate(self):
        ...
        for resource in self.package_dict['resources']:
            ...
            if resource['resource_type'] == 'external_link':
                resource = self.migrate_resource_external_link(resource, self.package_dict)
