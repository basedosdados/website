...
if not os.environ.get("CKAN_API_KEY"):
    print(
        "You need to export environment variable `CKAN_API_KEY`. PLease generate an api key from this page: http://localhost:5000/user/dev/api-tokens"
    )
    exit(1)
...
def check_package_by_name(package_name):
    ...
    package_dict["extras"] = [
        kv for kv in package_dict["extras"] if "key" in kv and "value" in kv
    ]  # there is a strange bug splitting extra dicts into dicts missing value and key, so we simply remove these, we aren't validating extras anyway
    try:
...
def check_all_packages(LIMIT=10):
    ...
    if errors:
        ...
        pprint(dataset)
        pprint(error.package_dict)
        print("--ERROR--")
        pprint(error.error_dict)
        ...
        if all_col_errors:
            ...
            all_col_errors = (
                all_col_errors.reset_index()
                .query("error_type != 'nan'")
                .set_index(["column", "error_type"])
            )
