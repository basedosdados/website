...
DO $$ BEGIN  
	...
	IF (SELECT to_regclass('public.backup_package') IS NULL) THEN
		CREATE TABLE public.backup_package AS SELECT * FROM package;
	END IF;
	IF (SELECT to_regclass('public.backup_package_extra') IS NULL) THEN
		CREATE TABLE public.backup_package_extra AS SELECT * FROM package_extra;
	END IF;
	IF (SELECT to_regclass('public.backup_resource') IS NULL) THEN
		CREATE TABLE public.backup_resource AS SELECT * FROM resource;
	END IF;
	IF (
		SELECT BOOL_OR(b.id IS NULL OR p.id IS NULL) FROM backup_package b FULL JOIN package p using(id)
	) THEN
		RAISE 'backup table exists and has different packages, please check manually';
	END IF;
	IF (
		SELECT BOOL_OR(b.id IS NULL OR r.id IS NULL) FROM backup_resource b FULL JOIN resource r using(id)
	) THEN
...
-- RESTORE_FROM_BACKUP
	INSERT INTO package  (SELECT * FROM backup_package)  ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, title = EXCLUDED.title, version = EXCLUDED.version, url = EXCLUDED.url, notes = EXCLUDED.notes, license_id = EXCLUDED.license_id, author = EXCLUDED.author, author_email = EXCLUDED.author_email, maintainer = EXCLUDED.maintainer, maintainer_email = EXCLUDED.maintainer_email, state = EXCLUDED.state, type = EXCLUDED.type, owner_org = EXCLUDED.owner_org, private = EXCLUDED.private, metadata_modified = EXCLUDED.metadata_modified, creator_user_id = EXCLUDED.creator_user_id, metadata_created = EXCLUDED.metadata_created;
	INSERT INTO resource (SELECT * FROM backup_resource) ON CONFLICT (id) DO UPDATE SET url = EXCLUDED.url, format = EXCLUDED.format, description = EXCLUDED.description, position = EXCLUDED.position, hash = EXCLUDED.hash, state = EXCLUDED.state, extras = EXCLUDED.extras, name = EXCLUDED.name, resource_type = EXCLUDED.resource_type, mimetype = EXCLUDED.mimetype, mimetype_inner = EXCLUDED.mimetype_inner, size = EXCLUDED.size, last_modified = EXCLUDED.last_modified, cache_url = EXCLUDED.cache_url, cache_last_updated = EXCLUDED.cache_last_updated, webstore_url = EXCLUDED.webstore_url, webstore_last_updated = EXCLUDED.webstore_last_updated, created = EXCLUDED.created, url_type = EXCLUDED.url_type, package_id = EXCLUDED.package_id, metadata_modified = EXCLUDED.metadata_modified;
	INSERT INTO package_extra (SELECT * FROM backup_package_extra) ON CONFLICT (id) DO UPDATE SET package_id = EXCLUDED.package_id, key = EXCLUDED.key, value = EXCLUDED.value, state = EXCLUDED.state;
