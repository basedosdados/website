                ...
                # break
                    ...
                    for column in resource["columns"]:
                        ...
                        if "name" not in column:
                            ...
                            "bigquery_type" not in column
                            or "description" not in column
                            or "has_sensitive_data" not in column
                            or "covered_by_dictionary" not in column
                        ):

                            l.append(
                                {
                                    package["name"]: "{}_{}".format(
                                        resource["name"], column["name"]
                                    )
                                }
                            )
            ...
            for column in resource["columns"]:
                ...
                if "has_sensitive_data" not in column:
                    ...
                    if column["has_sensitive_data"] is None:
                        if any(
                            substring in column["name"] for substring in substring_list
                        ):
                            column["has_sensitive_data"] = "no"
                        else:
                ...
                if (
                    ...
                    or column["covered_by_dictionary"] is None
                ):
...
def migrate_time_unit(package):
    ...
    for i, resource in enumerate(package["resources"]):
        if "time_unit" not in resource:
            ...
            elif resource["entity"] is not None and isinstance(
    ...
    for i, resource in enumerate(package["resources"]):
        if resource["resource_type"] == "bdm_table":
            if "code_url" in resource["data_cleaned_by"]:
                resource["data_cleaning_code_url"] = resource["data_cleaned_by"][
                    "code_url"
                ]
            else:
        ...
        if resource["name"] == "dicionario" and "spatial_coverage" in resource:
            ...
            if "spatial_coverage" in resource:
                if resource["spatial_coverage"] == "all":
                    ...
                    resource["spatial_coverage"]
                    == "bra,BA, CE, DF, MG, PA, PE, PR, RJ, SP"
                ):
                    resource["spatial_coverage"] = {
                        "continent": ["south_america"],
                        "country": ["bra"],
...
def migrate_spatial_coverage(package):
    ...
    for k, resource in enumerate(package["resources"]):
        ...
        if "spatial_coverage" in resource and resource["spatial_coverage"] not in [
            None,
            {},
        ]:

            if (
                "continent" in resource["spatial_coverage"]
                and resource["spatial_coverage"]["continent"] is None
            ):
                resource["spatial_coverage"] = None
            elif (
                "continent" in resource["spatial_coverage"]
                and resource["spatial_coverage"]["continent"] is not None
                and "all" in resource["spatial_coverage"]["continent"]
            ):
                resource["spatial_coverage"] = []
            elif resource["spatial_coverage"] == {
                "continent": ["south_america"],
                "country": None,
                "admin1": None,
                "admin2": None,
                ...
                resource["spatial_coverage"]["continent"] not in [None, []]
                and (
                    "country" in resource["spatial_coverage"]
                    and resource["spatial_coverage"]["country"] not in [None, []]
                )
                and (
                    "admin1" not in resource["spatial_coverage"]
                    or resource["spatial_coverage"]["admin1"] in [None, []]
                )
            ):
                sc = [
                    {
                ...
                (
                    "continent" in resource["spatial_coverage"]
                    and resource["spatial_coverage"]["continent"] not in [None, []]
                )
                and (
                    "country" in resource["spatial_coverage"]
                    and resource["spatial_coverage"]["country"] not in [None, []]
                ...
                and (
                    "admin1" in resource["spatial_coverage"]
                    and resource["spatial_coverage"]["admin1"] not in [None, []]
                )
                and (
                    "admin2" not in resource["spatial_coverage"]
                    or resource["spatial_coverage"]["admin2"] in [None, []]
                )
            ):
                ...
                if len(resource["spatial_coverage"]["admin1"]) >= 15:
                    sc.append(
                        {
                            "continent": resource["spatial_coverage"]["continent"][0],
                            "country": mapping[
                                resource["spatial_coverage"]["country"][0]
                            ],
                        }
                    )
                else:
                    for admin1 in resource["spatial_coverage"]["admin1"]:
                        sc.append(
                            {
                                "continent": resource["spatial_coverage"]["continent"][
                                    0
                                ],
                                "country": mapping[
                                    resource["spatial_coverage"]["country"][0]
...
def migrate_country_ip_address_required(package):
    ...
    for k, resource in enumerate(package["resources"]):
        if "country_ip_address_required" in resource and resource[
    ...
    if str in [type(element) for element in l]:
        ...
        for element in l:
            sls = [
                ...
                if element in sl
                and (
                    (len(stepsList(sl)) == 1 and stepsList(sl)[0] == step)
                    or len(sl) == 1
                )
            ]
        ...
        updated_temporal_coverage = sorted(
            [item for sublist in intervals for item in sublist]
        )
...
def migrate_temporal_coverage(package):
    ...
    for i, resource in enumerate(package["resources"]):
        ...
        if (
            "temporal_coverage" in resource
            and resource["temporal_coverage"] is not None
            and resource["temporal_coverage"] != []
        ):
            try:
                resource["temporal_coverage"] = migrate_temporal_coverage_field(
        ...
        if resource["resource_type"] == "bdm_table":
            for j, column in enumerate(resource["columns"]):
                if (
                    "temporal_coverage" in column
                    and column["temporal_coverage"] is not None
                    and column["temporal_coverage"] != []
                ):
                    column["temporal_coverage"] = migrate_temporal_coverage_field(
...
def fill_out_observation_level_from_identifying_columns(resource, sols):
    ...
    for identifying_column in resource["identifying_columns"]:
        ...
        if i is not None:
            ...
            if entity in [
                ol["entity"] for ol in resource["observation_level"] if "entity" in ol
            ]:
                ...
                for k, ol in enumerate(resource["observation_level"]):
                    if "entity" in ol and ol["entity"] == entity:
                        resource["observation_level"][k]["column"] = [
                            identifying_column
                        ]
    ...
    for i, resource in enumerate(package["resources"]):
        ...
        if resource["name"] == "dicionario":
            ...
            if "observation_level" in resource:
                ...
                if "identifying_columns" in resource and resource[
                    "identifying_columns"
                ] not in [None, []]:
                    ...
                    resource = fill_out_observation_level_from_identifying_columns(
                        resource, standard_observation_levels
                    )
    ...
    for k, resource in enumerate(package["resources"]):
        if resource["resource_type"] == "bdm_table":
            if "columns" in resource and resource["columns"] not in [None, []]:
                for j, column in enumerate(resource["columns"]):
                    if (
                        "measurement_unit" in column
                        and column["measurement_unit"] is not None
                    ):
                        column["measurement_unit"] = column["measurement_unit"].replace(
                            "2", "^2"
                        )
                        column["measurement_unit"] = column["measurement_unit"].replace(
                            "3", "^3"
